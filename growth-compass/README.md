...
| **Resume Import / 简历导入** | Extracts growth entries from your CV via LLM / 从简历中提取成长记录 | One-time / 一次性 |
| **Map / 图谱** | Builds skill graph with levels and patterns / 生成技能图谱 | Monthly / 每月 |
| **Compare / 对比** | Reveals hidden strengths — what evidence shows vs. what you claim / 揭示隐藏优势 | Any time / 随时 |
...
│   ├── comparison.py               # Skill comparison engine / 技能对比引擎
│   ├── i18n.py                     # Chinese/English locale / 中英文国际化
│   └── agent.py                    # Main orchestrator / 主编排器
├── tests/                          # Unit tests (30 tests) / 单元测试
│   ├── test_comparison.py
│   └── test_resume.py
├── demo/
...
| Resume import / 简历导入 | `capture_growth_moment.md` | Multi-entry extraction / 多条目提取 |
| View skill map / 查看技能图谱 | `map_skills.md` | Per-skill assessment + pattern detection |
| Skill comparison / 技能对比 | — (keyword matching) | Self-claimed vs. evidence-based |
...
## What's New in v1.1 / v1.1 更新

**Skill Comparison / 技能对比**: Compares what you say about yourself against what your journal evidence actually shows. Surfaces **Revealed** strengths you didn't know you had — the core "whoa" moment. 对比自我认知与实际证据，发现你自己都未意识到的隐藏优势。

**Resume Import / 简历导入**: Paste your CV or LinkedIn profile, and the LLM extracts every experience as a structured growth entry. Fastest way to seed your journal. 粘贴简历或领英资料，AI 自动提取每条经历为成长记录。

**Multi-Profile Support / 多用户支持**: Each person gets their own journal file. Switch profiles from the menu — no database needed. 每个人独立的日志文件，菜单内一键切换。

**Chinese by Default / 默认中文**: All UI text is Chinese. Switch to English with `$env:GROWTH_COMPASS_LANG="en"`. 界面默认中文，可通过环境变量切换英文。

**Tests / 测试**: 30 unit tests covering comparison engine and resume import. 30 个单元测试。
